...
const AppPage = () => {
  ...
  // Sync products and categories when user is loaded
  useEffect(() => {
    if (currentUser && navigator.onLine) {
      const syncData = async () => {
        try {
          console.log('[AppPage] Syncing products and categories...');
          await Promise.all([
            syncTable('products', 'products'),
            syncTable('categories', 'categories'),
            syncTable('transactions', 'transactions')
          ]);
          console.log('[AppPage] Sync complete, refreshing page data...');
          // Trigger a page refresh by dispatching a custom event
          window.dispatchEvent(new Event('productsUpdated'));
        } catch (error) {
          console.error('[AppPage] Error syncing data:', error);
        }
      };
      
      // Sync after a short delay to ensure user is fully loaded
