...
export function filterByUserId<T extends { userId?: string; userid?: string }>(
  ...
  if (!userId) {
    // If no userId, return all items (for products that don't have userId, show all)
    // This allows products synced from Supabase to be visible even without userId
    return items;
  }
  
  return items.filter(item => {
    // Check all possible userId field variations
    const itemUserId = item.userId || item.userid || (item as any).user_id || (item as any).userId;
    
    // If item has no userId at all, include it (for products shared across users)
    // This is important for products synced from mobile that might not have userId
    if (!itemUserId) {
      return true; // Include items without userId (they're considered shared)
    }
    
    // If item has userId, only include if it matches
