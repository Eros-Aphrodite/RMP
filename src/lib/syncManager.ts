...
function normalizeFromSupabase(obj: any): any {
  ...
  const columnMappings: Record<string, string> = {
    ...
    'checkout': 'checkOut',
    'createdat': 'createdAt',
    'user_id': 'userId', // Map user_id to userId for consistency
    'userid': 'userId', // Also handle userid
  ...
  for (const key of Object.keys(obj)) {
    ...
    // Also preserve original key if different (for backwards compatibility)
    if (normalizedKey !== key && !out[key]) {
      out[key] = obj[key];
    }
  }
